#!/usr/bin/env python3
"""
绿幕/洋红幕去除工具 - 移除序列帧中的绿色或洋红色像素（转为透明）。

用途：
  - 去除绿幕背景（chroma key green）
  - 去除洋红幕背景（chroma key magenta）
...
# 阶段2：边缘溢出清除
GREEN_LOW = 25           # 阻断通向角色内部的链路（robe greenness峰值40但大多<25）
MAGENTA_LOW = 25

# 距离变换参数
DISTANCE_THRESHOLD = 15  # 距角色>此值的透明区为"深背景"
SPILL_ITERATIONS = 12    # 泛洪轮数（确保从深背景到达角色边缘）
...
def magentaness(r: int, g: int, b: int) -> int:
    return min(r, b) - g
    ...
    for y in range(h):
        for x in range(w):
            ...
            if pixels[x, y][3] < 5:
                continue
            r, g, b, _ = pixels[x, y]
            if REMOVE_GREEN and greenness(r, g, b) >= GREEN_HIGH:
                mask[idx] = 1
            elif REMOVE_MAGENTA and magentaness(r, g, b) >= MAGENTA_HIGH:
                mask[idx] = 1
        ...
        for _ in range(len(queue)):
            ...
            for dx, dy in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ...
                if 0 <= nx < w and 0 <= ny < h:
                    ...
                    matched = False
                    if REMOVE_GREEN and greenness(r, g, b) >= GREEN_LOW:
                        matched = True
                    elif REMOVE_MAGENTA and magentaness(r, g, b) >= MAGENTA_LOW:
                        matched = True
...
def process_folder(input_dir: str, output_dir: str) -> None:
    ...
    print(f"绿色: greenness>={GREEN_HIGH}(直删) >={GREEN_LOW}(溢出)  [G-max(R,B)]")
    print(f"洋红: magentaness>={MAGENTA_HIGH}(直删) >={MAGENTA_LOW}(溢出)")
    print(f"深背景>{DISTANCE_THRESHOLD}px  泛洪{SPILL_ITERATIONS}轮")
    print("=" * 60)
