#!/usr/bin/env python3
"""
洋红幕去除工具 - 移除序列帧中的洋红色像素（转为透明）。

检测算法（两阶段 + 距离变换约束）：
  阶段1 — 高置信度：magentaness >= HIGH 直接移除
  阶段2 — 边缘溢出：从"深背景"（距角色>THRESHOLD px的透明区）队列泛洪。
           仅扩展 magentaness >= LOW 的不透明像素。

magentaness = min(R, B) - G，纯洋红(255,0,255)→255

用法：
  1. 把需要处理的PNG序列帧放入 input/ 文件夹
  2. 运行：python auto_remove_magenta.py
  3. 结果输出到 output/ 文件夹
"""
...
HIGH = 40       # 阶段1：magentaness >= 此值直接移除
LOW = 25        # 阶段2：magentaness >= 此值且邻近深背景时移除（越高越保护角色内部）
...
def build_mask(w: int, h: int, pixels, dist: list[int]) -> Image.Image:
    ...
    for y in range(h):
        for x in range(w):
            ...
            if pixels[x, y][3] < 5:
                continue
            if magentaness(pixels[x, y][0], pixels[x, y][1], pixels[x, y][2]) >= HIGH:
                mask[idx] = 1
...
def process_folder(input_dir: str, output_dir: str) -> None:
    ...
    print(f"magentaness>={HIGH}(直删) >={LOW}(溢出)  深背景>{DISTANCE_THRESHOLD}px  泛洪{SPILL_ITERATIONS}轮")
    print("=" * 60)
